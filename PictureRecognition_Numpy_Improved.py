...
""""This version has some changes, including the use of Numpy in more functions. Though, I still struggle using Numpy more significantly in the main 
function Model. However the program is already extremely fast without it."""
...
class Recognition():
    ...
    def normalize(self,x):
        ...
        """Input : x -> numpy arrays of arrays, contains the pictures
                   (p -> integer, by default, the pictures is coded with 255 possible pixel values)
    ...
    def model(self, x, y, h, w):
        ...
        for z in range(l): #loop on the labels
            ...
            p = 0
            #print(res0)
            threshold = (self.delta)*((counts[z])/self.batch)*100
    ...
    def prediction(self,image):
        ...
        image0 = np.zeros((self.h,self.w))
        for a in np.nditer(image0,op_flags=['readwrite']):
            if a > 0.5:
                a = 1
            else:
                a = -1
